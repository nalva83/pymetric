---
description: El orden de construcción — del milestone al plan con su tabla de piezas. Invoca la skill planner (fases de plan), que pregunta una vez (un bloque) y escribe docs/sdd/plans/active/. El detalle de cada spec lo escribe /specs.
...
- Ejecutá: leer el milestone → la ronda de preguntas (un solo bloque) → escribir el plan
  (`docs/sdd/plans/active/m<n>-<tema>.md`) con su tabla de piezas. **No** escribas el detalle
